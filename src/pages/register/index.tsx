...
export default function Register() {
  ...
  const handleRoleChange = (
    ...
    newRole: 'contractor' | 'supplier' | null
  ) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Chuyển hướng đến trang đăng nhập và truyền thông báo chúc mừng
      navigate('/login', {
        state: { message: 'Chúc mừng bạn đã đăng ký thành công!' },
      });
    } catch (err: any) {
      // Xử lý lỗi
        ...
        <Grid item xs={12} md={6} sx={{ p: 3 }}>
          ...
          >
            <ToggleButton
              value="contractor"
              sx={{
                fontWeight: role === 'contractor' ? 'bold' : 'normal',
                fontSize: '12px',
              }}
            >
              I'm Contractor
            ...
            <ToggleButton
              value="supplier"
              sx={{
                fontWeight: role === 'supplier' ? 'bold' : 'normal',
                fontSize: '12px',
              }}
            >
              I'm Supplier
